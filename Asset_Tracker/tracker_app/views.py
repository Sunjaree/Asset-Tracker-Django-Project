...
def handle_signup(request):
    if request.method == "POST":
        ...
        if User.objects.filter(company_name=company_name).count() == 1:
            messages.warning(
                request, "This company account already exists, Please login"
            )
...
def assign_assets(request, asset_id):
    company = request.user
    ...
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            # check if the asset is already assigned or not
            if asset.is_available == False:
                messages.warning(request, "Asset already assigned")
                return redirect("tracker_app")
